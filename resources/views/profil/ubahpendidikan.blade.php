        ...
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <hr class="my-3">
                    <form method="POST" action="{{ route('editPendidikan') }}" class="forms-sample">
                        @csrf
                        ...
                        <input type="hidden" name="itemNo" value="{{ request()->query('itemNo') }}">
                        <div class="row">
                            <div class="mb-3 col-md-7">
                                <label for="pendidikan" class="form-label">Pendidikan</label>
                                <input type="text" name="pendidikan" class="form-control" id="pendidikan" placeholder="Pendidikan" value="{{ $pendidikan->PENDIDIKAN }}" required>
                            </div>
                            <div class="mb-3 col-md-7">
                                <label for="keterangan" class="form-label">Keterangan</label>
                                <input type="text" name="keterangan" class="form-control" id="keterangan" placeholder="Keterangan" value="{{ $pendidikan->KETERANGAN }}" required>
                            </div>
                            <div class="mb-3 col-md-7">
                                <label for="status" class="form-label">Status</label>
                                <select class="form-select" id="status" name="status" aria-label="Default select example" required>
                                    <option value="T" {{ $pendidikan->Status == 'T' ? 'selected' : '' }}>Belum</option>
                                    <option value="Y" {{ $pendidikan->Status == 'Y' ? 'selected' : '' }}>Sudah</option>
                                </select>
                            </div>
                            <div class="mb-3 col-md-7">
                                <label for="tanggal" class="form-label">Tanggal</label>
                                <input type="text" class="form-control" id="tanggal" name="tanggal" placeholder="Tanggal" data-date-format="Y-m-d" value="{{ $pendidikan->Tanggal }}" required>
                            </div>
                        </div>
