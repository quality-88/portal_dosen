...
<meta name="csrf-token" content="{{ csrf_token() }}">
    ...
    <div class="col-md-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h4 class="mb-4">Konversi Nilai Mahasiswa</h4>
                <form class="row g-5" action="{{ route('viewTambahKonversi') }}" method="POST">
                    @csrf
                    <div class="col-md-4">
                        <label for="npm" class="form-label">NPM</label>
                        <div class="input-group">
                        <input type="text" class="form-control" id="npm" name="npm" placeholder="Masukkan Nama Mahasiswa disini" required>
                    ...
                    <div class="col-md-12">
                        <button type="submit" class="btn btn-primary btn-lg float-end">Cari</button>
                    </div>
                </form>
            </div>
        </div>
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <form class="row g-3">
                                            ...
                                            <div class="col-md-6">
                                                <label for="npmasal" class="form-label">NPM ASAL</label>
                                                <input type="text" class="form-control" id="npmasal" readonly>
                                            </div>
                                            <div class="col-md-6">
                                            ...
                                            <div class="col-md-6">
                                                <label for="KDPTASAL" class="form-label">ID PT. Asal</label>
                                                <input type="text" class="form-control" id="KDPTASAL" readonly>
                                            </div>
                                            <div class="col-md-6">
                                                <label for="IDPRODIASAL" class="form-label">ID Prodi Asal</label>
                                                <input type="text" class="form-control" id="IDPRODIASAL" readonly>
                                            </div>
                                            <div class="col-md-6">
                                                <label for="JenjangAkhir" class="form-label">Jenjang</label>
                                                <input type="text" class="form-control" id="JenjangAkhir" readonly>
                                            </div>
                                            <div class="col-md-6">
                                                <label for="ta" class="form-label">TA</label>
                                                <input type="text" class="form-control" id="ta" readonly>
                                            </div>
                                            <div class="col-md-6">
                                                <label for="ProdiAsal" class="form-label">Prodi Asal</label>
                                                <input type="text" class="form-control" id="ProdiAsal" readonly>
                                            </div>
                                        </form>
                                    </div>
                                </div>
...
#resultList li:last-child {
  border-bottom: none; /* Remove border from the last list item */
    ...
    if (searchQuery.length >= 4) {
        ...
        $.ajax({
            url: 'tambahkonversi/findMahasiswa',
            method: 'GET',
...
$(document).on('click', '#resultList li', function () {
    ...
    // Lakukan permintaan AJAX ke server untuk mencari Mahasiswa
    $.ajax({
        url: 'tambahkonversi/showInputKonversiNilai',
        method: 'GET',
        data: { npm: npm },
        success: function (data) {
            console.log('Server Response:', data);
            ...
            $('#KDPTASAL').val(mahasiswa.KDPTASAL);
            $('#IDPRODIASAL').val(mahasiswa.IDPRODIASAL);
            $('#JenjangAkhir').val(mahasiswa.JenjangAkhir);
            $('#ta').val(mahasiswa.ta);
            $('#ProdiAsal').val(mahasiswa.PRODIASAL);
        },
        error: function (error) {
            console.error('Error fetching data:', error);
        }
