...
<div class="page-content">
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="mb-0">Tunjangan Akademik</h4>
                    <hr class="my-4">
                    <form class="row g-5" action="{{ route('simpanTunjAkademik') }}" method="POST" id="tunjakademik">
                        @csrf
                        <div class="col-md-3">
    ...
    <div class="col-md-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h4 class="mb-0">Setting Tunjangan Akademik</h4>
                <hr class="my-4">
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                            @foreach($tunjakademik as $index => $j)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $j->ta }}</td>
                                    <td>{{ $j->semester }}</td>
                                    ...
                                    <td>
                                        <button class="btn btn-primary activateButton" data-ta="{{ $j->ta }}"
                                             data-semester="{{ $j->semester }}" data-statusdosen="{{ $j->statusdosen }}">Activate</button>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
...
<script>
            ...
            success: function(response) {
                ...
                if (response && response.success) {
                    ...
                    Swal.fire({
                        ...
                        window.location.href = "{{ route('showTunjAkademik') }}";
                    });
    ...
    $(document).ready(function () {
        $('.activateButton').click(function () {
            ...
            $.ajax({
                url: "{{ route('activateTunjAkademik') }}",
                method: 'POST',
                data: {
                    _token: "{{ csrf_token() }}",
                    ta: ta,
                ...
                success: function (response) {
                    if (response && response.success) {
                        ...
                        Swal.fire({
                            title: 'Error!',
                            text: response.error || 'Failed to update TUNJAKADEMIK',
                            icon: 'error',
                            confirmButtonText: 'OK'
                        });
                    }
                ...
                error: function (xhr, status, error) {
                    ...
                    Swal.fire({
                        title: 'Error!',
                        text: 'An error occurred while updating TUNJAKADEMIK',
                        icon: 'error',
                        confirmButtonText: 'OK'
                    });
                }
