...
class FungsionarisController extends Controller
{
    ...
    public function formKaprodi(Request $request)
    {
        ...
        return view('fungsionaris.fungsionaris', compact('allIdKampus','allProdi','allJabatan','allFakultas'));
    }
    public function insertJabatan(Request $request)
    {
        $data = $request->all();
        // Check if jabatanfungsi is not Kaprodi, clear prodi and idprodi
        if ($data['jabatanfungsi'] !== 'Kaprodi') {
            $data['prodi'] = null;
            $data['idprodi'] = null;
        }
        if ($data['jabatanfungsi'] !== 'Dekan') {
            $data['fakultas'] = null;
        ...
        if ($data['jabatanfungsi'] === 'Rektor') {
            // Insert data into Hak_Akses_Global table
            DB::table('Hak_Akses_Global')->insert([
        ...
        if (strpos($data['jabatanfungsi'], 'Dekan') === 0) {
            ...
            DB::table('fakultas')
                ->where('idfakultas', $data['idfakultas'])
                ->where('fakultas', $data['fakultas'])
                ->update([
                    'iddekan' => $data['id'],
                    'gelarpanjang' => $data['nama'], // id_dekan diupdate dengan id dari dekan yang baru
